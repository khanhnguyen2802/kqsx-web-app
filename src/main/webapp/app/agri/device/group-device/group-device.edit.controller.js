(function () {
    ...
    function GroupDeviceEditController($rootScope, $scope, $state,$stateParams,$timeout, $translate,
        ...
        var tableConfig = {
            ...
            loadFunction: Device.addToGroupDevice,     //api load du lieu
            columns: columns,               //bao gom cac cot nao
            handleAfterReload: null,        //xu ly sau khi load du lieu
            handleAfterReloadParams: null,  //tham so cho xu ly sau khi load
            deleteCallback: null,           //delete callback
            customParams: "tenantId==" + $scope.groupDevice.tenantId + ";gatewayId==" + $scope.groupDevice.gatewayId + ";groupDeviceId==" + $stateParams.groupDeviceId,               //dieu kien loc ban dau
        ...
        $scope.gatewayChange = function(){
            $timeout(function () {
                ...
                tableConfig.customParams = "tenantId==" + $scope.groupDevice.tenantId + "groupDeviceId==" + $stateParams.groupDeviceId + ";gatewayId==" + ($scope.groupDevice.gatewayId ? $scope.groupDevice.gatewayId : 0);
        ...
        GroupDevice.getFull($stateParams.groupDeviceId).then(function (data) {
            $scope.id = $stateParams.groupDeviceId;
            ...
            $scope.TABLES[tableConfig.tableId].customParams = "tenantId==" + $scope.groupDevice.tenantId + ";gatewayId==" + $scope.groupDevice.gatewayId + ";groupDeviceId==" + $stateParams.groupDeviceId;
            TableController.reloadPage(tableConfig.tableId);
        ...
        function updateGroupDevice(isClose){
            GroupDevice.update($scope.groupDevice).then(function(data){
                ...
                $timeout(function () {
                    isClose ? $state.go('group-devices'): $state.go('group-device-detail',{groupDeviceId: data.id});
                },1100);
                $scope.btnDisable = false;
            }).catch(function(data){
                if($scope.blockModal != null) $scope.blockModal.hide();
