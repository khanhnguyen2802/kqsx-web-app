(function () {
    ...
    function ComboBoxController($rootScope, Principal, $translate,apiData,$http, $scope) {
        ...
        function initFull(scope, config) {
            $scope = scope;
            ...
            $scope.ComboBox[config.id].config = {
                ...
                load: function (query, callback) {
                    ...
                    if (!$scope.ComboBox[config.id].totalCount || $scope.ComboBox[config.id].totalCount > ( ($scope.ComboBox[config.id].page - 1) * $scope.ComboBox[config.id].perPage)) {
                        if(!config.url || config.url === "") return;
                        var api = apiData.genApiFull(config.url, config.searchField, query.search, $scope.ComboBox[config.id].perPage, null, config.originParams, query.page, config.replaceUrl, config.orderBy);
                        $http.get(api).then(function (response) {
                            $scope.ComboBox[config.id].totalCount = parseInt(response.headers()["x-total-count"], 10);
                            callback(response.data);
                        });
        ...
        function initCustom(scope, config) {
            $scope = scope;
            ...
            $scope.ComboBox[config.id].config = {
                ...
                load: function (query, callback) {
                    ...
                    if (!$scope.ComboBox[config.id].totalCount || $scope.ComboBox[config.id].totalCount > ( ($scope.ComboBox[config.id].page - 1) * $scope.ComboBox[config.id].perPage)) {
                        if(!config.url || config.url === "") return;
                        var api = apiData.genApiCustom(config.url, config.searchField, query.search, $scope.ComboBox[config.id].perPage, null, config.originParams, query.page, config.replaceUrl, config.orderBy);
                        $http.get(api).then(function (response) {
                            $scope.ComboBox[config.id].totalCount = parseInt(response.headers()["x-total-count"], 10);
                            callback(response.data);
                        });
